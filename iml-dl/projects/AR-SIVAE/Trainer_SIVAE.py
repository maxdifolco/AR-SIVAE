...
from optim.losses.attributes_losses import compute_reg_loss
...
class PTrainer(Trainer):
    ...
    def __init__(self, training_params, model, data, device, log_wandb=True):
        ...
        self.factor = training_params['factor'] if 'factor' in training_params.keys() else 1.0
    ...
    def test(self, model_weights, test_data, task='Val', opt_weights=None, epoch=0):
        ...
        with torch.no_grad():
            for data in test_data:
                ...
                loss_rec = calc_reconstruction_loss(x_, x, loss_type='mse')
                if self.loss_type == 'pl':
