...
"""
    ...
    Pati et al., Attribute-based Regularization of Latent Spaces for Variational Auto-Encoders,
    Neural Computing and Applications, 2020
    Available here: https://github.com/ashispati/ar-vae/tree/master
"""

def eval_prediction(predictions, labels, num_classes, dict_classes, binary_label=True):

    if binary_label:
        acc, _ = mean_accuracy(predictions,labels)
        num_classes = num_classes + 1
    else:
        acc = accuracy(predictions, labels, average="macro", num_classes=num_classes)

    img_cm = plot_conf_mat(predictions, labels, num_classes, dict_classes)

    return acc.cpu().numpy().item(), img_cm
